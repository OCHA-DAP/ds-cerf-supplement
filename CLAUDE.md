...
- **`export_site_data.py`** → `site/review/data.json` (rows carry `kind: storm|drought`), served by static `site/review/index.html` (Storms/Droughts tabs) on GitHub Pages (`deploy-site.yml`, no commits to `main`). **Site layout = the team landing-page convention** (KB `methods/static-data-apps.md`): `site/index.html` is a hand-edited-cards landing page (HDX v2 `site/assets/site.css` + `hero.js`, copied from `ds-seas5-skill`), products live under their own paths (`/review/` — moved from `/` on 2026-09-18 — and `/mirror/`), and every product page starts with the back-to-home button. Adding a product = new directory + a card; never put a product at the root again.
...
GitHub Pages source must be **GitHub Actions** (not a branch). `site/review/data.json`, `site/mirror/meta.json` and `claude_work/` are git-ignored.
