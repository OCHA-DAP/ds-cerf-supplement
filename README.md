...
1. **Refresh OneGMS mirror** ([workflow](.github/workflows/refresh-mirror.yml)) — daily 05:30 UTC. Upserts the CERF feed into `aa.cerf_allocation` so newly-published allocations are matchable. On success it triggers the matcher(s).
2. **Match storms** ([workflow](.github/workflows/match-storms.yml)) — runs after the mirror. Stage 1 backfills SIDs that resolve unambiguously from the allocation title and opens a GitHub issue (tagging the maintainer) for the rest; stage 2 (Claude) researches the remainder and applies validated, high-confidence matches. More matchers (e.g. drought) can key off the same mirror refresh.
3. **Deploy site** ([workflow](.github/workflows/deploy-site.yml)) — runs after matching. A lightweight page showing which CERF allocation matched which storm, regenerated from the DB and deployed to Pages. → **https://ocha-dap.github.io/ds-cerf-supplement/**
...
Three chained workflows. `refresh-mirror` is the only one on a schedule; the rest
fire in order via `workflow_run` (so they run against a freshly-mirrored feed), and
each is also runnable on demand from the Actions tab.
...
Adding another matcher (drought, etc.) = a new workflow with the same
`workflow_run: [Refresh OneGMS mirror]` trigger.

### 3. Deploy site — `deploy-site.yml`

Runs after matching (plus on push to `site/**` and a daily 08:00 UTC backstop).
Rebuilds `site/data.json` from the DB and deploys to GitHub Pages.
